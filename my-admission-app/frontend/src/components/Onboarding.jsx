...
const Onboarding = ({ onComplete }) => {
  ...
  const updateGrade = (subject, value) => setGrades(prev => ({ ...prev, [subject]: value }));
  const addGradeRow = () => { const ns = ['Biology','Chemistry','History','Social Studies','Psychology','German','French']; const next = ns[extraSubjects.length % ns.length]; setExtraSubjects(prev => [...prev, next]); setGrades(prev => ({ ...prev, [next]: '' })); };
  ...
  const renderActiveStep = () => {
    ...
    switch (step) {
        ...
        return (
          <div className={commonClasses + " space-y-6"}>
            ...
            <div className="flex flex-wrap gap-2 max-h-60 overflow-y-auto">
              {SUBJECTS.filter(s => s.toLowerCase().includes(subjectFilter.toLowerCase())).map(sub => (
                <span key={sub} onClick={() => toggleSubject(sub)} className={`px-3 py-1.5 rounded-full text-sm cursor-pointer transition-all ${subjects.includes(sub) ? 'bg-ink text-white' : 'bg-white border border-ink/20 hover:border-ink'}`}>{sub}</span>
              ))}
            </div>
            {errorMsg && <div className="text-clay text-sm">{errorMsg}</div>}
            <div className="flex gap-3 mt-6">
        ...
        return (
          <div className={commonClasses + " space-y-6"}>
            ...
            <div className="grid grid-cols-2 gap-4 border border-ink/10 rounded-xl p-4 bg-white">
              <div><div className="font-mono text-[9px] text-ink-3 uppercase mb-1">What you want</div><div className="text-sm">People & ideas<br />Psychology<br />Research</div></div>
            ...
            <button onClick={() => setStep(8)} className="w-full bg-ink text-white rounded-pill py-3 font-semibold hover:bg-ink-2 transition-all">Add my grades →</button>
          ...
          <div className={commonClasses + " space-y-6"}>
            ...
            <div className="space-y-2">
              ...
              {extraSubjects.map(sub => (
                <div key={sub} className="flex items-center gap-3 py-2 border-b border-ink/10">
                  <span className="flex-1 text-sm">{sub}</span>
                  <input type="number" min="0" max="12" step="1" value={grades[sub] || ''} onChange={e => updateGrade(sub, e.target.value)} className="w-16 px-2 py-1 text-center border border-ink/20 rounded-lg" />
                </div>
              ))}
            </div>
      ...
      case 9: {
        if (programmesLoading) return <div className="text-center py-12"><div className="w-8 h-8 border-4 border-cream-3 border-t-ink rounded-full animate-spin mx-auto mb-4"></div><p>Loading programmes...</p></div>;
        const filtered = allProgrammes.filter(p => !programmeFilter || p.name?.toLowerCase().includes(programmeFilter.toLowerCase()) || p.uni?.toLowerCase().includes(programmeFilter.toLowerCase()));
        return (
          <div className={commonClasses + " space-y-6"}>
            <h2 className="font-serif italic text-3xl leading-tight text-ink">Which programmes are you thinking about?</h2>
            <img src="/Photos/Onboard%20target.svg" className="w-36 mx-auto" alt="" />
            <p className="text-ink-3 text-sm">Pick 1–3. These become your roadmap destinations.</p>
            <input type="text" placeholder="Search programmes..." value={programmeFilter} onChange={e => setProgrammeFilter(e.target.value)} className="w-full border border-ink/20 rounded-xl py-3 px-4 bg-white focus:outline-none focus:border-ink" />
            <div className="max-h-64 overflow-y-auto space-y-1">
              {filtered.map((prog, idx) => {
                const isSelected = selectedProgrammes.some(p => p.id === prog.id);
                return (
                  <div key={prog.id || idx} onClick={() => toggleProgramme(prog)} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all ${isSelected ? 'bg-ink/5 border border-ink' : 'border border-ink/10 hover:border-ink/30'}`}>
                    <div className={`w-4 h-4 rounded-full border-2 ${isSelected ? 'bg-ink border-ink' : 'border-ink-3'}`}></div>
                    <div><div className="font-medium text-sm">{prog.name}</div><div className="text-xs font-mono text-ink-3">{prog.uni} {prog.cutoff ? `· Q1 ${prog.cutoff}` : ''}</div></div>
                  </div>
                );
              })}
              {filtered.length === 0 && <div className="text-center text-ink-3 py-8">No programmes found.</div>}
            </div>
            <div className="text-right text-xs font-mono text-ink-3">{selectedProgrammes.length} of 3 selected</div>
            {errorMsg && <div className="text-clay text-sm">{errorMsg}</div>}
            <div className="flex gap-3 mt-6">
              <button onClick={prev} className="text-ink-3 hover:text-ink transition-colors">← Back</button>
              <button onClick={next} className="flex-1 bg-ink text-white rounded-pill py-3 font-semibold hover:bg-ink-2 transition-all">Continue →</button>
      ...
      case 10: {
        ...
        const avgGrade = (() => { const vals = Object.values(grades).filter(v => v !== '' && !isNaN(parseFloat(v))); return vals.length ? (vals.reduce((a,b) => a + parseFloat(b),0)/vals.length).toFixed(1) : '8.5'; })();
        const topCutoff = selectedProgrammes.length ? selectedProgrammes[0].cutoff : 11.0;
        return (
          <div className={commonClasses + " space-y-6"}>
            <h2 className="font-serif italic text-3xl leading-tight text-ink">Here's where you stand.</h2>
            <div className="grid grid-cols-2 gap-4 border border-ink/10 rounded-xl p-4 bg-white">
              <div><div className="font-mono text-[9px] text-ink-3 uppercase mb-1">What you want</div><div className="text-sm">People & ideas<br />Psychology<br />Research</div></div>
              <div><div className="font-mono text-[9px] text-ink-3 uppercase mb-1">Can reach</div>{selectedProgrammes.length ? selectedProgrammes.slice(0,3).map(p => <div key={p.id} className="text-sm flex justify-between"><span>{p.name.split(' ')[0]} · {p.uni.split(' ')[0]}</span><span className="font-mono">{p.cutoff}</span></div>) : <div className="text-sm text-ink-3 italic">Add grades or skip</div>}</div>
            </div>
            <div className="bg-cream-2 rounded-xl p-4">
              <div className="font-mono text-[9px] text-ink-3 uppercase mb-1">The gap</div>
              <p className="text-sm">You're at <strong>{avgGrade}</strong>. Your top target needs <strong>{topCutoff}</strong>. That's what this roadmap is built to close — one milestone at a time.</p>
            </div>
            <div className="border border-ink/10 rounded-xl p-4 bg-white italic text-ink-2">
              "Your interest in people and psychology is consistent across everything you've told me. The path to Psykologi is real — the variable is your Maths B grade. It's required at every university. That's where we start."
            </div>
